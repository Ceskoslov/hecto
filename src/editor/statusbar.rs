...
impl StatusBar {
    ...
    pub fn resize(&mut self, size: Size){
        ...
        if let Some(result) = size.height.checked_sub(self.margin_bottom).and_then(|result| result.checked_sub(1))
        {
            position_y = result;
            is_visible = true;
        }
    ...
    pub fn render(&mut self) {
        ...
        if let Ok(size) = Terminal::size() {
            ...
            debug_assert!(result.is_ok(), "Failed to render status bar: {:?}", result.err());
