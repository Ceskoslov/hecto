...
impl Editor {
    ...
    fn resize(&mut self, size: Size) {
        ...
        self.message_bar.resize(Size { height: 1, width: size.width });
        self.status_bar.resize(Size { height: 1, width: size.width });
        if let Some(command_bar) = &mut self.command_bar {
            command_bar.resize(Size { height: 1, width: size.width });
        }
    }
    ...
    fn show_prompt(&mut self){
        ...
        command_bar.resize(Size { height: 1, width: self.terminal_size.width });
    ...
    fn handle_quit(&mut self) {
        if !self.view.get_status().is_modified || self.quit_times+1 == QUIT_TIMES {
            ...
            self.message_bar.update_message(&format!("WARNING: File has unsaved changes. Press Ctrl-Q {} more times to quit without saving.", QUIT_TIMES - self.quit_times - 1));
            self.quit_times += 1;
        }
    }
